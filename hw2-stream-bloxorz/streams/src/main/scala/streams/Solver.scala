...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] = {
    val nei = for {
      (blk, mv) <- b.legalNeighbors
    } yield (blk, mv :: history)
    nei.toStream
  }
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    if(initial.isEmpty) Stream.empty
      ...
      val nextpaths = for{
        (blk, path) <- initial
        (nextblk, newpath) <- newNeighborsOnly( neighborsWithHistory(blk, path), explored)
      } yield(nextblk, newpath)
      val newexplored = (nextpaths map (_._1)).toSet
      nextpaths #::: from(nextpaths, explored.union(newexplored)) // `#:::` is stream concatenation
    }
  }
